...
public class UserRepositoryJdbcImpl implements UserRepository {
    ...
    @Override
    public List<User> findAllByProfileDescription(String profileDecription) {
        List<User> users = new ArrayList<>();
        try (Connection connection = dataSource.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement("select * from account where profileDescription = ?")) {
                statement.setString(1, profileDecription);
